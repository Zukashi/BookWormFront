...
export const OneRowInBookListAdmin = ({book, i, refresh }:any,) => {
    ...
    return (<>
        <tr className='h-16 font-normal text-[16px] max-h-[h-20] overflow-y-scroll'>
            ...
            <td className='p-3 border-[#dee2e6] border-[1px] '><p className='overflow-y-auto h-32 flex items-center'>{book.publishers.length !== 0 ? book.publishers.map((publisher:string) => <p>{publisher}</p>):<Link className='decoration-solid underline text-violet-600' to={`modify/${book._id}`}><p className='w-full h-full flex flex-col justify-center'>Add publishers</p></Link>}</p></td>
            <td className='p-3 border-[#dee2e6] border-[1px] '><div className='h-24 overflow-y-scroll '>{book.subjects.length !== 0 ? book.subjects.map((subject:string) => <p>{subject}</p>):<Link className='decoration-solid underline text-violet-600' to={`modify/${book._id}`}> <p className='flex flex-col justify-center w-full h-full'>Add subjects</p></Link>}</div></td>
