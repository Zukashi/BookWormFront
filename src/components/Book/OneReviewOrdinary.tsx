...
export const OneReviewOrdinary = (props:any) => {
    ...
    return (<>
        <div className='ml-[1.5rem]'>
            ...
            {props.review?.description && !props.review.spoilers &&   <div className={` max-h-[6rem] overflow-hidden  font-[450] ${showFullText ? 'overflow-auto max-h-screen': props.review.description.length > 160 ? 'overflow-hidden relative before:content-[""] before:absolute before:h-12 before:w-full before:bottom-0               before:bg-gradient-to-b before:from-transparent before:to-white ' : ''} `}><p className='text-[1rem] font-[450] mt-3 ' >{props.review?.description}</p></div>}
