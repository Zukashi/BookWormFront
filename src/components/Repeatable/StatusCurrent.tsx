                    ...
                    <div className={` w-screen  absolute left-1/2 -translate-x-1/2 md:bottom-1/2 md:translate-y-1/2 w-screen bg-white rounded-xl pb-10 max-w-[410px]  ${!modal  ? '-bottom-[268px]' : '-bottom-2'}`}>
                        <div className='w-[85%] mx-auto'>
                            ...
                            <div className='flex flex-col gap-3'>
                                {Object.keys(statusObjectToMap).map((status:string) => <div className={`${ statusUnformatted === status && 'bg-[#ccc]'} border-2 border-[#271c148f] rounded-3xl px-2 py-1.5 cursor-pointer flex active:bg-[#bbb] transition-all  duration-300 hover:bg-[#ddd]`} onClick={() => updateStatusOfBook(status)}><button className='w-full '><span className='font-medium flex justify-center items-center'>{statusUnformatted === status &&
