...
export const BooksSearchBar = () => {
  ...
  return (<>
    { category === 'author' ? <div className='w-screen absolute top-[5vh] flex flex-col items-center'>
      {search.Result.slice(0,1).map(search => <Link to={`/author/${search.author_key}`} state={search.author_key[0]}><div className='flex gap-4 w-32 cursor-pointer hover:text-lime-400 '>
      ...
      {search.Result.slice(0,4).map(search => search.cover_i &&
        ...
        <Link to={`/author/${search.author_key}`} state={search.author_key}> {<h3 className='hover:text-lime-400 w-20'>{search.author_name}</h3>}
        </Link>
      </div>)}
    </div> }
