...
export const OneBookOlSearch = ({book,refresh}:Props) => {
    ...
    console.log(typeof book.author_name !== 'undefined' && book.author_name[0])
    return (<>
        <div className='flex relative  gap-[1rem]   sm:h-[100%] mt-5    justify-center mb-4   sm:gap-[.5rem] items-start'> <div className='   inline-block  '>
            <div  onClick={AddBookToDb} className='relative   '><div className='flex items-center bg-black rounded-xl'><img ref={refImg} aria-label={`click this image to redirect to book ${book.title}` }  src={`https://covers.openlibrary.org/b/isbn/${book.isbn[0]}-L.jpg?default=false`} onError={(e) => {
