import React, {ChangeEvent, useEffect, useRef, useState} from 'react';
import {Link, Route, useNavigate} from "react-router-dom";
import {Id, toast, ToastContainer } from 'react-toastify';
import {useAxiosPrivate} from "../../hooks/useAxiosPrivate";
import {useToast} from "@chakra-ui/react";

interface Props {
    book: any,
    refresh: () => void,
};
// if(typeof book.isbn === 'object') {
//     console.log(olBooks.length)
//     if( book.isbn[0].length >= 10){

export const OneBookOlSearch = ({book,refresh}:Props) => {
    const refImg = useRef<HTMLImageElement>(null);
    const axiosPrivate = useAxiosPrivate();
    const stars = Array(5).fill(0);
    const toastChakra  = useToast()
    const navigate = useNavigate();
    const [loading ,setLoading] = useState<boolean>(false);
    const AddBookToDb = async () => {
        setLoading(true);
        const id = toast.loading("Please wait...",{
            theme:'dark',
            position:'bottom-center'
        })
        try{
            const res =  await axiosPrivate.post('http://localhost:3001/book',JSON.stringify({
                isbn:book.isbn[0]
            }))

            toast.update(id, { render: "Book has been added to db", type: "success", isLoading: false,position:'bottom-center',theme:'dark', autoClose:3500 });
            toastChakra({
                title: 'Success',
                description: "Book has been added to the database",
                status: 'success',
                duration: 5000,
                isClosable: true,
            })
            navigate(`/book/${res.data}`);

        }catch(e:any){
            setLoading(false);
            if(e.response.status === 409){
                const res = await axiosPrivate.get(`http://localhost:3001/book/isbn/${book.isbn[0]}`);
                navigate(`/book/${res.data._id}`)
            }
            toast.update(id, { render: e?.response?.data, type: "error", isLoading: false,position:'bottom-center',theme:'dark', autoClose:3500 });

        }


    }

    if(!book){
        return <h1>123</h1>
    }
    return (<>
        <div className='flex relative  gap-[1rem]   sm:h-[100%] mt-5    justify-center mb-4   sm:gap-[.5rem] items-start'> <div className='   inline-block  '>
            <div  onClick={AddBookToDb} className='relative   '><div className='flex items-center bg-black rounded-xl'><img ref={refImg} aria-label={`click this image to redirect to book ${book.title}` }  src={`https://covers.openlibrary.org/b/isbn/${typeof book.isbn ==='object' && book.isbn[0].length >= 10 && book.isbn[0]}-L.jpg?default=false`} onError={(e) => {
                e.currentTarget.src = "data:image/png;base64,"
            }} className="cursor-pointer  inline-block  cursor-default w-[35vw] max-w-[165px] min-h-[190px] rounded-xl  sm:w-[120px] sm:min-h-[180px] sm:h-[100%] md:h-64  md:w-[170px] xl:h-72 xl:min-w-[185px] hover:bg-black hover:opacity-50"   alt=""/></div>

            </div>

        </div>

            <div className='  flex flex-col gap-6  w-[153px]   '><p className='w-full  text-[15px] md:h-14 font-bold w-36 leading-5  sm:text-[14px] md:text-[20px] sm:h-[40px] sm:flex sm:items-start max-h-[140px]
    '>{book.title}</p>
                <p className='text-[16px] sm:text-[16px] w-full '>{typeof book.author_name !== 'undefined' && book.author_name[0]} </p>
               <div className='flex items-center mt-4'>
                   <div className=' h-8    mr-2 flex     items-center'>
                       {
                           stars.map((_, index) => {
                               return (
                                   <i className={`fa-solid fa-star  text-xl sm:text-[16px]   ${(book?.ratings_sortable?.toFixed(2) ? book?.ratings_sortable?.toFixed(2): 0)- 1 + 0.01 > index  && `text-[#faaf00]`} mb-2 sm:mb-0` } key={index}></i>

                               )
                           })

                       }



                   </div>
                   <span className='flex  gap-6 font-medium'>
                    {(book?.ratings_sortable?.toFixed(2) && book?.ratings_sortable?.toFixed(2))}
     </span>
               </div>



            </div>



        </div>


    </>)
}